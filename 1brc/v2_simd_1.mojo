...
fn parse_station[simd_width: Int](data: Span[UInt8], mut start: Int, end: Int) raises -> List[(String, Int)]:
    alias middle = ord(';')
    alias new_line = ord('\n')
    alias NEG = ord('-')
    alias ZERO = ord('0')
    ...
    while newlines != 0:
        ...
        if relevant_semicolons != 0:
            ...
            var city = String(bytes=data[start + start_of_line_idx : start + Int(semicolon_idx)])
            ...
            var val = sign * (val_short * Int(is_short) + val_long * Int(not is_short))
...
fn v2(file_path: String) raises -> String:
    ...
    while position < end:
        ...
        if i > N:
            break
    return String("V2, Assab: ", d["Assab"], ", Detroit: ", d["Detroit"],  ", Veracruz: ", d["Veracruz"])
